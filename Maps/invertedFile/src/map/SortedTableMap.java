...
public class SortedTableMap<K,V> extends AbstractSortedMap<K,V> {
    ...
    private class LazyIterable implements Iterable<Entry<K,V>>{
        public Iterator<Entry<K,V>> iterator() {return new LazyIterator();}
    }

    private class LazyIterator implements Iterator<Entry<K,V>>{
        private int current = 0;
        public boolean hasNext(){return current < table.size();}
        ...
        public void remove(){table.remove(current);}
    }

    public Iterable<Entry<K, V>> lazyEntrySet(){
